...
export function elementToJson(node) {
  ...
  if (isShouldRenderAttrs) {
    ...
    Array.from(node.attributes).forEach(attr => {
      switch (attr.name) {
        case 'style':
          ...
          break;
        case 'class':
          if (node.classList && node.classList.length > 0) {json.classList = [...node.classList];}
          break;
        default:
    ...
    if (node.childNodes && node.childNodes.length > 0) {json.children = [...node.childNodes].map(cn => elementToJson(cn)).filter(v => v);}
  }
